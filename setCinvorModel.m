...
%set noise variables
m.tau = tau;
m.eps = epsilon; %single neuron noise
m.rho = rho;
...
for iStimVal = 1:e.stimLevel
    voxelResponses = zeros(e.trialPerStim,nVoxels);
    for itrial = 1:e.trialPerStim
      ...
      %neuralResponse = neuralResponse + randn([nVoxels,neuronsPerVox])*m.eps;
      ...
      voxelResponses(itrial,:) = voxelResponse;
    end
    m.voxelResponse{iStimVal}= voxelResponses;
end
